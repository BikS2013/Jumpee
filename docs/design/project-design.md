...
3. **MenuBarController** - The menu bar and visual workspace UI coordinator:
   - Shows a system display symbol and the current space's custom name (or "Desktop N" if unnamed)
   - Opens a transient visual workspace popover from the status item or global hotkey
   - Supplies display-grouped desktop data, current-desktop context, feature status, and action callbacks to the popover
...
5. **JumpeeSettingsWindowController** - Native macOS Settings:
...
6. **RenameDesktopPanelController** - Focused rename workflow:
...
7. **OverlayManager** - Desktop watermark:
...
2. User presses Cmd+J or clicks the status item -> visual workspace popover opens showing searchable, display-grouped desktops
3. User clicks a desktop row -> popover closes (focus is not handed back to the previous app, v1.9.3) -> `SpaceNavigator.navigate(bySteps:)` posts Ctrl+Left/Right once per desktop on the active display (v1.9.4; Ctrl+N only for a desktop on another display) -> space switches
4. Space changes -> `SpaceDetector.getCurrentSpaceIndex()` called for display position, `getCurrentSpaceID()` called for config key lookup -> name looked up by space ID -> menu bar title, popover snapshot, and overlay updated
...
Jumpee follows a visual workspace utility model: the status item opens a transient native popover containing current-desktop context, search, display-grouped desktop rows, shortcuts, and the three primary contextual actions. Persistent preferences remain in a standard auxiliary Settings window. The Settings toolbar is noncustomizable and remains visible across General, Appearance, Shortcuts, and Advanced panes. Each pane has a purpose-specific size and title.

The interface remains AppKit-based and dependency-free. `Sources/main.swift` retains the application services and popover orchestration; `Sources/WorkspacePopover.swift` owns the visual workspace popover; `Sources/SettingsUI.swift` owns the Settings window, panes, live preview, and shortcut recorder; `Sources/RenamePanel.swift` owns the rename interaction. `build.sh` compiles every Swift source under `Sources/` into the existing universal binary.
...
Jumpee grabs the focused window's title bar with a synthesized mouse drag (Amethyst 0.22+ technique) and, while the button is held, synthesizes the Mission Control "Move left a space" / "Move right a space" shortcuts (Ctrl+Left / Ctrl+Right, symbolic hotkeys 79 / 81) once per desktop between the current and the target desktop. Since v1.9.5 the presses are target-driven (`SpaceNavigator.pressSpaceArrow(route:logTag:completion:)`, shared with desktop navigation). `MenuBarController.desktopRoute(toGlobalPosition:)` builds a `DesktopRoute`: every space ID of the active display in left-to-right order (`SpaceDetector.allSpaceIDsInOrder(onDisplay:)`, including full-screen app spaces, which Ctrl+Left/Right also steps through), the target space ID and the planned step count. Before every press Jumpee reads `CGSGetActiveSpace`, stops when it is the target, and otherwise presses towards the target; it then polls until the active space changes (up to 2.5 s), waits 200 ms for the switch to settle and re-plans. At most 2 x steps + 3 presses are sent. A dropped press is simply sent again, and a switch that registers late cannot overshoot, because every press is planned from the desktop actually active. v1.9.4 counted presses instead and re-sent one that had not switched within 1 s; a switch normally registers 0.55-0.6 s after the press, so under load the re-sent press added an extra desktop and the switch overshot. Until v1.9.3 the presses were a fixed 450 ms apart, and macOS dropped a press that arrived while the previous switch animation was still running, so multi-desktop moves stopped part-way. The mouse is released 400 ms after the last switch and the window lands on the destination desktop, which macOS follows. `MenuBarController.desktopSteps(toGlobalPosition:)` converts the chosen desktop into a signed step count on the active display; destinations on another display are refused (the drag cannot cross displays).
...
### Popover Keyboard Selection, ⌃N Hint and Reset Dock (v1.9.6)

`WorkspacePopoverContentViewController` keeps `visibleRows` (row view + item, in list order, rebuilt by `rebuildDesktopRows()`) and `selectedIndex`. As the search field's delegate it implements `control(_:textView:doCommandBy:)`: `moveDown:` / `moveUp:` move the selection (clamped; from no selection, Down picks the first row and Up the last), `insertNewline:` calls `onNavigate` with the selected item's global position. `controlTextDidChange` selects the first match while a filter is typed. `WorkspacePopoverRowView.isSelected` draws a 1.5-point accent border over a 24% accent fill, and `select(_:)` scrolls the row into view. `hintLabel` (11 pt, secondary colour, between the list and the upper separator; the list shrank from 286 to 262 points) reads "Tip: ⌃1–⌃9 jump straight to a desktop from any app · ↑↓ select" without a selection and "⌃N jumps straight to “name” from any app · ↩ switches now" with one (N = global position, the macOS "Switch to Desktop N" numbering). The footer gains a Reset Dock button (`arrow.clockwise`) wired through `onResetDock` to `WorkspacePopoverController`, which closes the popover and calls `SpaceNavigator.restartDock()` (`/usr/bin/killall Dock`; launchd relaunches it). `WorkspacePopoverController` records `CGSGetActiveSpace` in `show(...)` and, in `popoverDidClose`, re-activates the previous app only if that space is still active on close and 0.3 s later.
