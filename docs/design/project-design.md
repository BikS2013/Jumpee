...
Jumpee grabs the focused window's title bar with a synthesized mouse drag (Amethyst 0.22+ technique) and, while the button is held, synthesizes the Mission Control "Move left a space" / "Move right a space" shortcuts (Ctrl+Left / Ctrl+Right, symbolic hotkeys 79 / 81) once per desktop between the current and the target desktop. Since v1.9.4 the presses are closed-loop (`SpaceNavigator.pressSpaceArrow(steps:logTag:completion:)`, shared with desktop navigation): after each press Jumpee polls `CGSGetActiveSpace` until the active desktop changes, waits 200 ms for the switch to settle, then sends the next press; a press that has not switched within 1 s is re-sent, up to 3 times per step. Until v1.9.3 the presses were a fixed 450 ms apart, and macOS dropped a press that arrived while the previous switch animation was still running, so multi-desktop moves stopped part-way. The mouse is released 400 ms after the last switch and the window lands on the destination desktop, which macOS follows. `MenuBarController.desktopSteps(toGlobalPosition:)` converts the chosen desktop into a signed step count on the active display; destinations on another display are refused (the drag cannot cross displays).
